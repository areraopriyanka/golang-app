...
func (suite *IntegrationTestSuite) TestTransactionAchPull() {
	...
	reason := "Settlements"
	payloadData := ledger.BuildOutboundAchDebitRequest(
		&userRecord,
		"50040002699049",
		"50000",
		"DEBTOR",
	...
	transactionRequest := handler.TransactionAchPullRequest{
	...
	e := handler.NewEcho()
	req := httptest.NewRequest(http.MethodPost, "/ach/pull", bytes.NewReader(requestBody))
	...
	err = h.TransactionAchPull(customContext)
	...
	var responseBody handler.TransactionAchPullResponse

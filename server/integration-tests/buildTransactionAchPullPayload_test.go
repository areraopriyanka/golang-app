...
type buildTransactionAchPullContext struct {
...
func (suite *IntegrationTestSuite) TestBuildTransactionAchPullPayload() {
	defer SetupMockForLedger(suite).Close()
	ctx := suite.newBuildTransactionAchPullContext()
	note := "Settlement"
	buildPayloadRequest := handler.BuildTransactionAchPullPayloadRequest{
	...
	suite.AssertSuccessBuildTransactionAchPullPayload(ctx, buildPayloadRequest)
}

func (suite *IntegrationTestSuite) TestBuildTransactionAchPullPayloadWithoutNote() {
	defer SetupMockForLedger(suite).Close()
	ctx := suite.newBuildTransactionAchPullContext()
	buildPayloadRequest := handler.BuildTransactionAchPullPayloadRequest{
		AmountCents:       model.TransferCents(5000),
...
func (suite *IntegrationTestSuite) newBuildTransactionAchPullContext() buildTransactionAchPullContext {
	h := suite.newHandler()
	...
	return buildTransactionAchPullContext{
		handler:           h,
		user:              userRecord,
		token:             token,
...
func (suite *IntegrationTestSuite) callBuildTransactionAchPull(ctx buildTransactionAchPullContext, buildPayloadRequest handler.BuildTransactionAchPullPayloadRequest) *httptest.ResponseRecorder {
	...
	mw.POST("/accounts/ach/pull/build", ctx.handler.BuildTransactionAchPullPayload)
	req := httptest.NewRequest(http.MethodPost, "/accounts/ach/pull/build", bytes.NewReader(requestBody))
...
func (suite *IntegrationTestSuite) AssertSuccessBuildTransactionAchPullPayload(ctx buildTransactionAchPullContext, buildPayloadRequest handler.BuildTransactionAchPullPayloadRequest) {
	rec := suite.callBuildTransactionAchPull(ctx, buildPayloadRequest)
	...
	var payload ledger.OutboundAchDebitRequest
	...
	suite.Require().Equal("ACH_PULL", payload.TransactionType, "TransactionType should be ACH_PULL")
	suite.Require().Equal("5000", payload.TransactionAmount.Amount, "Amount should match")
	suite.Require().Equal(ctx.user.FirstName, payload.Creditor.FirstName, "Creditor Name should match")
	...
	suite.Require().NoError(err)
	suite.Require().Equal(userAccountCard.AccountNumber, payload.CreditorAccount.Identification, "Creditor Account Name should match")
	// "9900009606" and "011401533" come from mockoon; we do not store the debtor's account number in the middleware
	suite.Require().Equal("9900009606", payload.DebtorAccount.Identification, "Debtor Account number should match")
	suite.Require().Equal("011401533", payload.DebtorAccount.Institution.Identification, "Debtor routing number should match")
}

func (suite *IntegrationTestSuite) TestBuildTransactionAchPullPayloadWithMaxAmount() {
	defer SetupMockForLedger(suite).Close()
	ctx := suite.newBuildTransactionAchPullContext()
	buildPayloadRequest := handler.BuildTransactionAchPullPayloadRequest{
		AmountCents:       model.TransferCents(10000002),
		DreamFiAccountID:  "217140",
...
func (suite *IntegrationTestSuite) TestBuildTransactionAchPullPayloadNSF() {
	defer SetupMockForLedger(suite).Close()
	ctx := suite.newBuildTransactionAchPullContext()
	buildPayloadRequest := handler.BuildTransactionAchPullPayloadRequest{

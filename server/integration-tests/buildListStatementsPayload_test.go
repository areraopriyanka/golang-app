...
func (suite *IntegrationTestSuite) TestBuildListStatementsPayload() {
	...
	request := request.BuildListStatementsPayloadRequest{
		PageNumber: 1,
		PageSize:   5,
	}
	...
	req := httptest.NewRequest(http.MethodPost, "/list-statements/build", bytes.NewReader(requestBody))
	...
	err = handler.BuildListStatementsPayload(customContext)

...
// @accept json
// @produce json
// @param buildTransactionAchPullPayloadRequest body BuildTransactionAchPullPayloadRequest true "Request body to request a transaction"
...
// @router /account/accounts/ach/pull/build [post]
func (h *Handler) BuildTransactionAchPullPayload(c echo.Context) error {
	...
	var requestData BuildTransactionAchPullPayloadRequest
	...
	availableBalanceCents := externalAccountRecord.AvailableBalanceCents
	// TODO: some methods that Plaid uses for account linking (e.g., micro-deposits) do not work with Plaid Balance
	// see: https://support.plaid.com/hc/en-us/articles/14977532310167-Can-I-obtain-Balance-and-Identity-data-for-Items-created-using-Same-Day-Micro-deposits
	// This might be a good use-case for Plaid Signal.
	// Captured in DT-1032 and DT-1205
	if availableBalanceCents == nil {
		logger.Error("could not determine if external account has sufficient funds for transfer")
		return response.InternalServerError("could not determine if external account has sufficient funds for transfer", errtrace.New(""))
	}
	hasSufficientFunds := *availableBalanceCents-int64(requestData.AmountCents) >= 0
	if !hasSufficientFunds {
		logger.Error("external account does not have sufficient funds for transfer", "transfer amount cents", requestData.AmountCents, "availableBalanceCents", *availableBalanceCents)
		return response.GenerateErrResponse(constant.INSUFFICIENT_FUNDS, "", "external account does not have sufficient funds for transfer", http.StatusUnprocessableEntity, errtrace.New(""))
	}

	var plaidItemRecord dao.PlaidItemDao
	err = db.DB.Model(dao.PlaidItemDao{}).Where("user_id=? AND plaid_item_id=?", userId, externalAccountRecord.PlaidItemID).Find(&plaidItemRecord).Error
	if err != nil {
	...
	if externalAccountRecord.PrimaryOwnerName == nil {
		...
		if err != nil {
			...
			err = db.DB.Model(dao.PlaidAccountDao{}).Where("user_id=? AND id=?", userId, externalAccountID).Find(&externalAccountRecord).Error
			if err != nil {
	...
	payload := ledger.BuildOutboundAchDebitRequest(
		userRecord,
	...
	logger.Info("created ach pull payload", "userID", userRecord.Id)

	return c.JSON(http.StatusOK, payloadResponse)
}

type BuildTransactionAchPullPayloadRequest struct {
